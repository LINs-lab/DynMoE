...
def eval_model(args):
    ...
    # OURS
    if args.get_gating_params:
        from moellava.utils import get_routing_records, get_total_tokens, get_sim_mat, get_gates
        records_hooks = get_routing_records(model)
        token_hooks = get_total_tokens(model)
        sim_mat_hooks = get_sim_mat(model)
        gates_hooks = get_gates(model)
    ...
    if args.get_gating_params:
        records_tokens = {}
        cnt = 0
        for record_hook, token_hook, sim_mat_hook, gates_hook in zip(records_hooks, token_hooks, sim_mat_hooks, gates_hooks):
            records_tokens[cnt] = {}
            records_tokens[cnt]['record'] = record_hook.routing_records
            records_tokens[cnt]['token'] = token_hook.total_tokens
            records_tokens[cnt]['smi_mat'] = sim_mat_hook.sim_matrix
            records_tokens[cnt]['gates'] = gates_hook.gates
            cnt += 1
        
        task = args.image_folder.split('/')[1]
        torch.save(records_tokens, f'{model_name}_{task}_gating_data.pt')
...
if __name__ == "__main__":
    ...
    parser.add_argument("--get_gating_params", action="store_true")
